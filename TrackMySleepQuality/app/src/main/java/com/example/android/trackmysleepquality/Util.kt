...
private val ONE_MINUTE_MILLIS = MILLISECONDS.convert(1, MINUTES)
private val ONE_HOUR_MILLIS = MILLISECONDS.convert(1, HOURS)
...
fun convertDurationToFormatted(startTimeMilli: Long, endTimeMilli: Long, res: Resources): String {
    ...
    return when {
        durationMilli < ONE_MINUTE_MILLIS -> {
            val seconds = SECONDS.convert(durationMilli, MILLISECONDS)
            res.getString(R.string.seconds_length, seconds, weekdayString)
        }
        durationMilli < ONE_HOUR_MILLIS -> {
            val minutes = MINUTES.convert(durationMilli, MILLISECONDS)
            res.getString(R.string.minutes_length, minutes, weekdayString)
        }
        else -> {
            val hours = HOURS.convert(durationMilli, MILLISECONDS)
